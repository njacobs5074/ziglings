...
pub fn main() void {
    ...
    const meal = food_loop: for (menu) |food| {
        ...
        required: for (food.requires) |required, required_ingredient| {
            ...
            for (wanted_ingredients) |want_it| {
                if (required_ingredient == want_it) continue :required;
            } else continue :food_loop;

            // We did not find this required ingredient, so we
            // can't make this Food. Continue the outer loop.
            continue :food_loop;
        } else break food; // Required ingredients were all wanted for this Food
    } else menu[0]; // Default item
